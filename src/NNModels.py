...
class SimpleMLPRegressor(base.BaseModel):
    def __init__(self, config: Dict) -> None:
        ...
        for unit in config['model']['units']:
            model.add(
                tf.keras.layers.Dense(unit, activation=config['model']['activation'])
            )
        ...
        optimizer_arg = {
            i: config['optimizer'][i] for i in config['optimizer'] if i != 'optimizer'
        }

        if optimizer_type == 'sgd':
            optimizer = tf.keras.optimizers.SGD(**optimizer_arg)
        elif optimizer_type == 'adam':
            optimizer = tf.keras.optimizers.Adam(**optimizer_arg)
        else:
            err_msg = 'ignore optimizer. passed {}'.format(optimizer_type)
            raise ValueError(err_msg)

        model.compile(
            optimizer, loss=config['model']['loss'], metrics=config['model']['metrics']
        )
    ...
    def _fit(
        ...
        reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(**self.config['reduce_lr'])
...
class SimpleMLPClassifier(base.BaseModel):
    def __init__(self, config: Dict) -> None:
        ...
        model.add(
            tf.keras.layers.Dense(config['model']['n_class'], activation='softmax')
        )
